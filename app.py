...
def load_data():
    ...
    car_data['condition'] = car_data['condition'].map(traducciones_condicion).fillna(car_data['condition'])
...
filtered_data = filtered_data[
    (filtered_data['model_year'] >= rango_año[0]) & (filtered_data['model_year'] <= rango_año[1])
]
...
if modelo_seleccionado != 'Todos':
    filtered_data = filtered_data[filtered_data['model'] == modelo_seleccionado]
...
mostrar_dispersion = st.checkbox('Gráfico de Dispersión: Precio vs Kilometraje')
mostrar_histograma_condicion = st.checkbox('Histograma de la Condición')
mostrar_barras_modelo = st.checkbox('Gráfico de Barras: Precio Promedio por Modelo')
st.markdown("---")
...
if filtered_data.empty:
    ...
    if mostrar_histograma_año:
        ...
        fig1.update_layout(title_font_size=16, title_x=0.5, plot_bgcolor='white', paper_bgcolor='white')
  ...
  if mostrar_dispersion:
    ...
    if mostrar_histograma_condicion:
        ...
        fig3.update_layout(title_font_size=16, title_x=0.5, plot_bgcolor='white', paper_bgcolor='white', showlegend=False)
    ...
    if mostrar_barras_modelo:
        ...
        precio_promedio_modelo = filtered_data.groupby('model')['price'].mean().reset_index().sort_values('price', ascending=False)
